...
void EventSystem::UpdateSfEvents() {
    ...
    while (window.pollEvent(event)) {
        sfEvents.insert(
            std::pair<sf::Event::EventType, sf::Event>(event.type, event));
    }
}
const std::unordered_multimap<sf::Event::EventType, sf::Event>&
EventSystem::GetSfEvents() const {
    return sfEvents;
}
std::pair<EventSystem::ConstIterator, EventSystem::ConstIterator>
EventSystem::GetSfEvents(const sf::Event::EventType type) const {
