...
namespace MaxrEngine
{
	...
	void TransformComponent::SetWorldInfoFrom(const Matrix2D& transform) const
	{
		...
		scale.x = std::sqrt(transformMatrix[0][0] * transformMatrix[0][0] + transformMatrix[1][0] * transformMatrix[1][0]);
		scale.y = std::sqrt(transformMatrix[0][1] * transformMatrix[0][1] + transformMatrix[1][1] * transformMatrix[1][1]);

		rotation = std::atan2(transformMatrix[0][1], transformMatrix[0][0]) * 180 / PI;
	}
	...
	void TransformComponent::SetLocalInfoFrom(const Matrix2D& transform) const
	{
		...
		localScale.x = std::sqrt(transformMatrix[0][0] * transformMatrix[0][0] + transformMatrix[1][0] * transformMatrix[1][0]);
		localScale.y = std::sqrt(transformMatrix[0][1] * transformMatrix[0][1] + transformMatrix[1][1] * transformMatrix[1][1]);

		localRotation = std::atan2(transformMatrix[0][1], transformMatrix[0][0]) * 180 / PI;
	}
