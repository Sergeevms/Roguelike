...
void GameWorld::LateUpdate() {
    for (int i = static_cast<int>(markedToDestroyGameObjects.size()) - 1;
         i >= 0; --i) {
...
void MaxrEngine::GameWorld::DestroyGameObjectImmediate(
    std::shared_ptr<GameObject> gameObject) {
    gameObjects.erase(
        std::remove_if(gameObjects.begin(), gameObjects.end(),
                       [gameObject](std::shared_ptr<GameObject> obj) {
