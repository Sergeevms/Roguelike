...
constexpr float oneDegreeInRadians =
    std::numbers::pi_v<float> / (fullRotation * 0.5F);
...
// NOLINTBEGIN(bugprone-easily-swappable-parameters) : direct matrix
// constraction from values
...
float Matrix2D::GetMinor(const int row, const int column) const {
    ...
    float minor = (submatrix[0][0] * submatrix[1][1]) -
