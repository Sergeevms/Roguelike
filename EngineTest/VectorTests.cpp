...
TEST(VectorTests, Angle) {
    ...
    EXPECT_FLOAT_EQ(MaxrEngine::Angle(vectorF3, vectorF2),
                    std::numbers::pi_v<float> / 4.f);
    EXPECT_FLOAT_EQ(MaxrEngine::Angle(vectorF1, vectorF4),
                    std::numbers::pi_v<float> / 3.f);
    EXPECT_FLOAT_EQ(MaxrEngine::Angle(vectorF2, vectorF4),
                    std::numbers::pi_v<float> / 6.f);
    EXPECT_FLOAT_EQ(MaxrEngine::Angle(vectorF1, vectorF5),
...
TEST(VectorTests, Normalize) {
    ...
    MaxrEngine::Vector2Df normalizedZeroVector =
        MaxrEngine::Normalized(zeroVector);
