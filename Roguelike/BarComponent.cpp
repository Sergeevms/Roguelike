...
namespace Roguelike
{
	Roguelike::BarComponent::BarComponent(MaxrEngine::GameObject* gameObject, MaxrEngine::Vector2Df centerOffset, MaxrEngine::Vector2Df barSize, sf::Color barColor, float borderSize, float maxAmount)
		: Component(gameObject), centerOffset(centerOffset), barSize(barSize), barColor(barColor), borderSize(borderSize), maxAmount(maxAmount), currentAmount(maxAmount)
	{
	}
	...
	void BarComponent::Render()
	{
		...
		auto objectPosition = gameObject->GetComponent<MaxrEngine::TransformComponent>()->GetWorldPosition();
		auto barTopLeft = objectPosition + centerOffset - barSize * 0.5f;
		MaxrEngine::Vector2Df filledBarSize = { barSize.x * currentAmount / maxAmount, barSize.y };
	...
	void BarComponent::SetCenterOffset(const MaxrEngine::Vector2Df& newCenterOffset)
	{
		centerOffset = newCenterOffset;
	}

	MaxrEngine::Vector2Df BarComponent::GetBarSize() const
	{
		return barSize;
	}
