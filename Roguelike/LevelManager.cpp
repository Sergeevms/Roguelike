...
void LevelManager::LoadNextLevel() {
    ...
    for (const auto& condition : levelParameters.completeConditions) {
        conditions.emplace_back(
            LevelCompleteCondition::CreateCondition(currentLevel, condition));
...
void Roguelike::LevelManager::RegisterLevel(
    const GameLevel::Parameters& parameters) {
    gameLevelParameters.emplace_back(parameters);
}
std::vector<std::shared_ptr<LevelCompleteCondition>>&
LevelManager::GetCompleteConditions() {
    return conditions;
}
void LevelManager::GenerateLevel() {
    GameLevel::Parameters newLevelParameters =
        Settings::Instance()->firstLevelParameters;
